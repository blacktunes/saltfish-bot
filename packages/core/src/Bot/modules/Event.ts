    ...
    if (this.showLog) {
      this.log('message.private', (event) => {
        ...
        .log('notice.group_upload', (event) => {
          ...
          const user_name = this.Bot.Data.groupMemberList[event.group_id]
            ? white(this.Bot.Data.groupMemberList[event.group_id][event.user_id] || '')
            : ''
          const user_id = white(event.user_id.toString())
          const filename = white(event.file.name)
          this.Bot.Log.logNotice(
            `群${group_name}(${group_id}) - ${user_name}(${user_id}) 上传了 ${filename}`,
            'EVENT'
          )
        })
        .log('notice.group_admin', (event) => {
          ...
          const user_name = this.Bot.Data.groupMemberList[event.group_id]
            ? this.Bot.Data.groupMemberList[event.group_id][event.user_id] || ''
            : ''
          const user_id = event.user_id.toString()
          if (event.sub_type === 'set') {
            this.Bot.Log.logNotice(
              `群${white(group_name)}(${white(group_id)}) - ${white(user_name)}(${white(
                user_id
              )}) 成为了管理员`,
              'EVENT'
            )
            ...
            this.Bot.Log.logWarning(
              `群${magenta(group_name)}(${magenta(group_id)}) - ${white(user_name)}(${magenta(
                user_id
              )}) 被取消了管理员`,
              'EVENT'
            )
        ...
        .log('notice.group_decrease', (event) => {
          ...
          const user_name = this.Bot.Data.groupMemberList[event.group_id]
            ? magenta(this.Bot.Data.groupMemberList[event.group_id][event.user_id] || '')
            : ''
          const user_id = magenta(event.user_id.toString())
          const operator_name = this.Bot.Data.groupMemberList[event.group_id]
            ? magenta(this.Bot.Data.groupMemberList[event.group_id][event.operator_id] || '')
            : ''
          const operator_id = magenta(event.operator_id.toString())
          if (event.sub_type === 'kick') {
            this.Bot.Log.logWarning(
              `群${group_name}(${group_id}) - ${user_name}(${user_id}) 被 ${operator_name}(${operator_id}) 踢出群聊`,
              'EVENT'
            )
          } else if (event.sub_type === 'leave') {
            this.Bot.Log.logWarning(
              `群${group_name}(${group_id}) - ${user_name}(${user_id}) 退出了群聊`,
              'EVENT'
            )
          } else if (event.sub_type === 'kick_me') {
            this.Bot.Log.logWarning(
              `群${group_name}(${group_id}) - ${red(
                this.Bot.Data.name
              )} 被 ${operator_name}(${operator_id}) 踢出群聊`,
              'EVENT'
            )
          }
        })
        .log('notice.group_increase', (event) => {
          ...
          const user_id = white(event.user_id.toString())
          const operator_name = this.Bot.Data.groupMemberList[event.group_id]
            ? white(this.Bot.Data.groupMemberList[event.group_id][event.operator_id] || '')
            : ''
          const operator_id = white(event.operator_id.toString())
          const type = event.sub_type === 'invite' ? '邀请' : '同意'
          this.Bot.Log.logNotice(
            `群${group_name}(${group_id}) - ${operator_name}(${operator_id}) ${type} ${user_name}(${user_id}) 加入群聊`,
            'EVENT'
          )
        })
        .log('notice.group_ban', (event) => {
          ...
          const user_id = event.user_id.toString()
          const operator_name = this.Bot.Data.groupMemberList[event.group_id]
            ? this.Bot.Data.groupMemberList[event.group_id][event.operator_id] || ''
            : ''
          const operator_id = event.operator_id.toString()
          if (event.sub_type === 'ban') {
            this.Bot.Log.logWarning(
              `群${magenta(group_name)}(${magenta(group_id)}) - ${magenta(user_name)}(${magenta(
                user_id
              )}) 被 ${magenta(operator_name)}(${magenta(operator_id)}) 禁言 ${magenta(
            ...
            this.Bot.Log.logNotice(
              `群${white(group_name)}(${white(group_id)}) - ${white(user_name)}(${white(
                user_id
              )}) 被 ${white(operator_name)}(${white(operator_id)}) 解除禁言`,
              'EVENT'
            )
          }
        })
        ...
        .log('notice.group_recall', (event) => {
          ...
          this.Bot.Log.logWarning(
            `群${group_name}(${group_id}) - ${operator_name}(${operator_id}) 撤回了${
              event.operator_id === event.user_id ? '' : ` ${user_name}(${user_id}) 的`
            }一条消息 (${message_id})`,
            'EVENT'
          )
        })
        .log('notice.friend_recall', (event) => {
          ...
          const message_id = magenta(event.message_id.toString())
          this.Bot.Log.logWarning(
            `${user_name}(${user_id}) 撤回了一条消息 (${message_id})`,
            'EVENT'
          )
        })
        .log('notice.notify', (event) => {
          ...
          const user_id = white(event.user_id.toString())
          const target_name =
            event.sub_type !== 'honor' && this.Bot.Data.groupMemberList[event.group_id]
              ? white(this.Bot.Data.groupMemberList[event.group_id][event.target_id] || '')
              : ''
          const target_id = event.sub_type !== 'honor' ? white(event.target_id.toString()) : ''
          if (event.sub_type === 'poke') {
            this.Bot.Log.logNotice(
              `群${group_name}(${group_id}) - ${user_name}(${user_id}) 戳了戳 ${target_name}(${target_id})`,
              'EVENT'
            )
          } else if (event.sub_type === 'lucky_king') {
            this.Bot.Log.logNotice(
              `群${group_name}(${group_id}) - ${target_name}(${target_id}) 获得了 ${user_name}(${user_id}) 发出的群红包运气王`,
              'EVENT'
            )
          } else if (event.sub_type === 'honor') {
            const honor =
              event.honor_type === 'talkative'
                ? '龙王'
                : event.honor_type === 'performer'
                ? '群聊之火'
                : event.honor_type === 'emotion'
                ? '快乐源泉'
                : '群荣誉'
            this.Bot.Log.logNotice(
              `群${group_name}(${group_id}) - ${user_name}(${user_id}) 获得了 ${honor}`,
              'EVENT'
            )
          }
        })
        .log('notice.group_card', (event) => {
          ...
          this.Bot.Log.logNotice(
            `群${group_name}(${group_id}) - ${oldcard}(${user_id}) 将群名片修改为${newcard}`,
            'EVENT'
          )
        })
        .log('notice.offline_file', (event) => {
          ...
          this.Bot.Log.logNotice(
            `${user_name}(${user_id}) 发送了离线文件 ${filename}(${url})`,
            'EVENT'
          )
        })
        ...
        .log('request.group', async (event) => {
          ...
          if (event.sub_type === 'add') {
            this.Bot.Log.logNotice(
              `(${user_id}) 申请加入 ${group_name}(${group_id}) 验证消息：${comment}`,
              'EVENT'
            )
          } else if (event.sub_type === 'invite') {
            this.Bot.Log.logNotice(
              `(${user_id}) 邀请${this.Bot.Data.name}加入 ${group_name}(${group_id}) 验证消息：${comment}`,
              'EVENT'
            )
          }
        })
          ...
          if (event.sub_type === 'add') {
            this.Bot.Log.logNotice(
              `(${white(event.sender_id.toString())}) 的消息被 (${white(
                event.operator_id.toString()
              )}) 设为精华消息 (${white(event.message_id.toString())})`,
              'EVENT'
            )
          } else {
            this.Bot.Log.logWarning(
              `(${magenta(event.sender_id.toString())}) 的消息被 (${magenta(
                event.operator_id.toString()
              )}) 取消精华消息 (${magenta(event.message_id.toString())})`,
              'EVENT'
            )
          }
        })
